...
import { collection, onSnapshot, orderBy, query, getDocs, updateDoc, doc } from "firebase/firestore";
        ...
        <section className="mt-3">
            <div className="flex items-center justify-between my-10">
                <h3 className="font-[mulish] text-2xl font-semibold text-gray-700 tracking-wide">
                    {`All Orders (${String(ordersWithItems.length).padStart("2", 0)}) `}
                </h3>
                        ...
                        <div key={order.id}  className="p-4 shadow-md w-auto rounded-md bg-[#f6f6f6]">
                            ...
                            <div className="flex gap-2">
                                ...
                                <button disabled={updating || order?.status === "Rejected" || order?.status === "Completed"} onClick={()=> handleRejectOrder(order.id)} className={`flex-1  px-3 py-2 rounded text-xs transition ${order?.status === "Rejected" ? "bg-red-700 text-white cursor-not-allowed" : order?.status === "Completed" ? "bg-[#016d2c] cursor-not-allowed text-white" : order?.status === "Pending" ? "bg-[#FFB900] text-black cursor-pointer" : "" } disabled:opacity-70`}>
                                   {order.status === "Pending" ? "Reject Order" : order.status}
                                </button>
                            </div>
                        </div>)
                    })}
         ...
         {showModal && selectedOrder && (
            <section>
                <div className={`bg-[#f4f4f4] w-[40%] h-full absolute right-0 overflow-y-auto`}>
                    <div className="flex items-center justify-between sticky top-0 px-4 py-2 bg-[#f4f4f4] z-20">
                        <div className="flex items-center gap-2">
                    ...
                    <div className="px-4">
                        {detailLoading ? (
                            ...
                            <>
                                ...
                                <div className="w-full h-[44px] my-5 rounded-md">
...
{/* <button className="bg-blue-600 text-white py-3 px-6 font-[mulish] text-sm rounded-md tracking-wide cursor-pointer">Accept Order</button> */}
                                    {/* <button className="bg-red-600 text-white py-3 px-6 font-[mulish] text-sm rounded-md tracking-wide cursor-pointer">Reject Order</button> */}

                                    {/* <button disabled={updating || selectedOrder?.status === "Rejected" || selectedOrder?.status === "Completed"} onClick={()=> handleRejectOrder(selectedOrder.id)} className={`flex-1  px-3 py-2 rounded text-xs transition ${selectedOrder?.status === "Rejected" ? "bg-red-700 text-white cursor-not-allowed" : selectedOrder?.status === "Completed" ? "bg-[#016d2c] cursor-not-allowed text-white" : selectedOrder?.status === "Pending" ? "bg-[#FFB900] text-black cursor-pointer" : "" } disabled:opacity-70`}>
                                   Reject 
                                </button> */}