...
function ShownShoeProduct(){
   ...
   const [selectedShoeProduct, setSelectedShoeProduct] = useState(null)
   ...
   const handleEditProduct = (product) =>{
      setSelectedShoeProduct(product)
   ...
   const handleUpdateProduct = async (e) => {
      ...
      setIsLoading(true)
      try {
         const productRef = doc(db, "shoeproductDB", selectedShoeProduct.id);
   ...
   const handleDelete = async (productId) => {
      ...
      try {
         await deleteDoc(doc(db, "shoeproductDB", productId));
   ...
   return(<>
      ...
      {editingModal && selectedShoeProduct && (
         <div className="w-[40%] h-screen bg-white absolute right-0 z-60 overflow-y-auto">
            <div className="flex items-center justify-between px-4 py-3 bg-white shadow sticky top-0">
               <h3 className="capitalize font-[mulish] text-lg tracking-wide font-semibold text-blue-600">#Editing {selectedShoeProduct.productName}</h3>
             ...
             <div className="p-4">
               ...
               <form onSubmit={handleUpdateProduct}>
                  ...
                  {/* <div className="w-full h-[44px] my-3"> 
                     {isLoading ? (<button>Loading......</button>) : (<button className="cursor-pointer bg-blue-600 text-white w-full h-full rounded-md">Update</button>)}
                  </div> */}
      ...
      <div className={`w-full h-screen bg-[#000000ab] fixed top-0 z-50 transition-all duration-300  ${detailModal ? "right-0" : "right-[-100%]"}`}>
        {detailModal && selectedShoeProduct &&(
            ...
            <div className="w-[1200px] h-[550px] bg-white rounded-md overflow-y-auto">
               ...
               <div className="flex items-center justify-around mt-10">
                  <div className="w-[40%] h-[400px]">
                     <img src={selectedShoeProduct.productImage} alt={selectedShoeProduct.productName} className="w-full h-full object-cover"/>
                  </div>
                  <div className="w-[40%]">
                     <h3 className="text-2xl uppercase font-[mulish] font-semibold text-gray-600 tracking-wide">{selectedShoeProduct.productName}</h3>
                     <p className="flex items-center my-3 text-xl font-[mulish] font-bold">Price: <TbCurrencyNaira size={20}/> {Number(selectedShoeProduct.productPrice).toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</p>
                     <div className="w-full p-1 h-[300px] overflow-hidden">
                        <p className="text-base font-[lato] leading-[25px] w-full h-full overflow-y-auto tracking-wide custom-scrollbar">{selectedShoeProduct.productDescription || "No product description"}</p>
                     </div>
                  </div>
               </div>
            </div>
